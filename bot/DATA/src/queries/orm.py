from sqlalchemy import text, insert, select, and_, or_, desc, func, case
...
class SyncORM:
    ...
    @staticmethod
    def select_conditers_with_lazy_relationship():
        with session_factory() as session:
            query = (select(ConditersORM))

            res = session.execute(query).scalars().all()

            conditer_1_product = res[0].fproducts
    ...
    def test():
        with session_factory() as session:
            ...
            res = session.execute(query).scalars().all()
            print('!!!', res[0].fcanmake)
            # print('!!!', res[0].fproducts[0])
            # print('!!!', res[0].fproducts[0].fcakeparts)
            # print('!!!', res[0].fproducts[0].fcakeparts[1].fingrcomb)
            # print('!!!', res[0].fproducts[0].fcakeparts[1].fingrcomb.fcake_type_more)
            # print('!!!', res[0].fproducts[0].fcakeparts[1].fingrcomb.fcake_ingr_more)
            # print('!!!', res[0].fproducts[0].fcakeparts[1].fingrcomb.fingr_taste_more)
    ...
    def get_result(filter_cake):
        ...
        with session_factory() as session:
            ...
            return dict(map(lambda x: (x[0][0], x[0][1]), sorted(prods.items(), key=sort_prods_with_filter)))
